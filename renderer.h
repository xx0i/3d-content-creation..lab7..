...
#include <ktxvulkan.h>
#include "../3dcc-lab-7-xx0i/Materials/TextureUtilsKTX.h"
...
public:
	...
	void loadImageFromGltf()
	{
		...
		UploadKTXTextureToGPU(vlk, "C:/full sail/3d content creation/3dcc-lab-7-xx0i/PBR IBL ENV/diffuse.ktx2", textures[size + 1].textureHandle,
			textures[size + 1].textureData, textures[size + 1].image, textures[size + 1].imageView);
		UploadKTXTextureToGPU(vlk, "C:/full sail/3d content creation/3dcc-lab-7-xx0i/PBR IBL ENV/specular.ktx2", textures[size + 2].textureHandle,
...
private:
	...
	{
		std::string fragmentShaderSource = ReadFileIntoString("../Materials/FragmentShader_PBR.hlsl");   //("../FragmentShader.hlsl");
